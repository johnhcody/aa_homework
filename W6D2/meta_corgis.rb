...
class MetaCorgiSnacks
  ...
  #   if method_name.start_with?("bone") || method_name.start_with?("kibble") || method_name.start_with?("treat")
  #     info = @snack_box.get_bone_info(@box_id)
  #     tastiness = @snack_box.get_bone_tastiness(@box_id)
  #     result = "Bone: #{info}: #{tastiness} "
  #     tastiness > 30 ? "* #{result}" : result
  ...
  def self.define_snack(name)
    define_method(name) 
    name = name.to_s
    if name.start_with?("bone") || name.start_with?("kibble") || name.start_with?("treat")
      info = @snack_box.get_bone_info(@box_id)
      tastiness = @snack_box.get_bone_tastiness(@box_id)
      result = "Bone: #{info}: #{tastiness} "
      tastiness > 30 ? "* #{result}" : result
